...
fn test_total_liquidity() {
    ...
    for pool_fee in CONSTANT_PRODUCT_FEE_AVAILABLE {
        ...
        setup.router.deposit(&user1, &tokens, &pool_hash, &30000, &0);
    }
    ...
    // );
    // e.cost_estimate().budget().print();
    assert!(e.cost_estimate().budget().cpu_instruction_cost() < 100_000_000, "budget exceed");
}
...
fn test_constant_product_pool() {
    ...
    router.deposit(&user1, &tokens, &pool_hash, &desired_amount, &0);
...
fn test_simple_ongoing_reward() {
    ...
    assert_eq!(standard_liquidity.add(&stable_liquidity), U256::from_u32(&e, 370));
    ...
    reward_token.approve(&admin, &router.address, &((stable_pool_tps as i128) * 60), &9999);
    assert_eq!(
    ...
    assert_eq!(router.get_total_outstanding_reward(&tokens, &stable_pool_hash), 0);
    assert_eq!(reward_token.balance(&standard_pool_address) as u128, standard_pool_tps * 60);
    assert_eq!(reward_token.balance(&stable_pool_address) as u128, stable_pool_tps * 60);

    assert_eq!(router.claim(&user1, &tokens, &standard_pool_hash), standard_pool_tps * 30);
    assert_eq!(router.claim(&user1, &tokens, &stable_pool_hash), stable_pool_tps * 30);
    ...
    assert_eq!(router.get_total_claimed_reward(&tokens, &stable_pool_hash), stable_pool_tps * 30);
    assert_eq!(
        router.get_total_configured_reward(&tokens, &stable_pool_hash),
        stable_pool_tps * 60
    );
    ...
    assert_eq!(router.get_total_claimed_reward(&tokens, &stable_pool_hash), stable_pool_tps * 60);
    assert_eq!(
        router.get_total_configured_reward(&tokens, &stable_pool_hash),
        stable_pool_tps * 60
    );
...
fn test_rewards_distribution() {
    ...
    let (stable_pool_hash1, stable_pool_address1) = router.init_stableswap_pool(
        &user1,
        &tokens1,
        &10
    );
    let (stable_pool_hash2, stable_pool_address2) = router.init_stableswap_pool(
        &user1,
        &tokens2,
        &10
    );
    ...
    assert_eq!(router.get_total_outstanding_reward(&tokens1, &stable_pool_hash1), 0);
    assert_eq!(router.get_total_outstanding_reward(&tokens2, &standard_pool_hash2), 0);
    assert_eq!(router.get_total_outstanding_reward(&tokens2, &stable_pool_hash2), 0);
    ...
    let stable_liquidity1 = router.get_total_liquidity(&tokens1).sub(&standard_liquidity1);
    let stable_liquidity2 = router.get_total_liquidity(&tokens2).sub(&standard_liquidity2);
    assert_eq!(standard_liquidity1.add(&stable_liquidity1), U256::from_u32(&e, 370));
    assert_eq!(standard_liquidity2.add(&stable_liquidity2), U256::from_u32(&e, 370));
    ...
    assert_eq!(reward_token.balance(&stable_pool_address1) as u128, stable_pool_tps * 60);
    assert_eq!(
        reward_token.balance(&standard_pool_address2) as u128,
        standard_pool_tps * 60 + 2000
    );
    assert_eq!(reward_token.balance(&stable_pool_address2) as u128, stable_pool_tps * 60 + 2000);
...
fn test_rewards_distribution_override() {
    ...
    let (stable_pool_hash, _stable_pool_address) = router.init_stableswap_pool(
        &user1,
        &tokens,
        &10
    );
