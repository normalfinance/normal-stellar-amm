...
fn test_deposit() {
    ...
    setup.contract.deposit(&setup.router.address, &setup.token_a.address, &amount_to_deposit);
    ...
    assert_eq!(setup.contract.get_reserve(&setup.token_a.address), Reserve {
...
fn test_request_payout() {
    ...
    setup.contract.request_payout(&setup.admin, &setup.token_a.address, &amount_to_payout);
    ...
    assert_eq!(setup.contract.get_reserve(&setup.token_a.address), Reserve {
        ...
        last_payout_ts: setup.env.ledger().timestamp(),
    });

    // Buffer last payout timestamp updated
    assert_eq!(setup.contract.get_last_payout_timestamp(), setup.env.ledger().timestamp());
}
...
fn test_withdraw_surplus() {
    ...
    setup.contract.withdraw_surplus(&setup.admin, &setup.token_a.address, &amount_to_witdraw);

    let expected_balance = reserve_before.balance * setup.contract.get_min_reserve_ratio();

    // Buffer has min reserve ratio
    assert_eq!(setup.token_a.balance(&setup.contract.address), expected_balance as i128);
    // Buffer reserve updates
    assert_eq!(setup.contract.get_reserve(&setup.token_a.address), Reserve {
...
fn test_sync() {
    ...
    setup.token_a_admin_client.mint(&setup.contract.address, &(excess_token_amount as i128));
...
fn test_request_payout_from_not_router() {
    let setup = Setup::default();
    let user = Address::generate(&setup.env);
    setup.contract.request_payout(&user, &setup.token_a.address, &100_0000000_u128);
}
...
fn test_deposit_invalid_token() {
    ...
    setup.contract.deposit(&setup.admin, &bogus_token, &100_0000000_u128);
}
...
fn test_deposit_over_max() {
    let setup = Setup::default();
    setup.contract.set_reserve_max_balance(&setup.admin, &setup.token_a.address, &99_0000000_u128);
    setup.contract.deposit(&setup.admin, &setup.token_a.address, &100_0000000_u128);
}
...
fn test_request_payout_too_soon() {
    ...
    setup.contract.request_payout(&setup.admin, &setup.token_a.address, &100_0000000_u128);

    // 10 seconds
    jump(&setup.env, 10);

    setup.contract.request_payout(&setup.admin, &setup.token_a.address, &10_0000000_u128);
}
...
fn test_request_payout_insufficient_balance() {
    let setup = Setup::default();
    setup.contract.request_payout(&setup.admin, &setup.token_a.address, &100000_0000000_u128);
}
