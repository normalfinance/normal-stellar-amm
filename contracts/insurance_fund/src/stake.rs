...
use normal_rust_types::types::{InsuranceFundReserve, Stake};
...
pub trait StakeExt {
    fn new(user: Address, token: Address) -> Self;
    fn save(&mut self, e: &Env);
    fn validate_base(&self, e: &Env);
    fn checked_shares(&self, e: &Env) -> u128;
    fn unchecked_shares(&self) -> u128;
    fn increase_shares(&mut self, e: &Env, delta: u128);
    fn decrease_shares(&mut self, e: &Env, delta: u128);
    fn update_shares(&mut self, e: &Env, new_shares: u128);
}

impl StakeExt for Stake {
    fn new(user: Address, token: Address) -> Self {
        Stake {
            user,
            token,
...
//     let token = Address::(&setup.env, "token");    
