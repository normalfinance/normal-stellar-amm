...
use crate::constants::{CONSTANT_PRODUCT_FEE_AVAILABLE, ELASTIC_MAX_POOLS};
use crate::testutils;
...
fn test_stableswap_pools_amount_over_max() {
    ...
    // init constant product pools to make sure we don't affect elastic counter
...
fn test_privileged_users() {
    ...
    assert_eq!(
        privileged_addrs,
        testutils::elastic_pool::Client::new(&e, &stable_address).get_privileged_addrs()
    );
}
