use crate::constants::{ELASTIC_MAX_POOLS, MAX_POOLS_FOR_PAIR};
...
pub fn get_elastic_pool_hash(e: &Env) -> BytesN<32> {
    ...
    match e.storage().instance().get(&DataKey::ElasticPoolHash) {
        Some(v) => v,
        None => panic_with_error!(&e, PoolRouterError::ElasticHashMissing),
    }
}
...
pub fn add_pool(
    ...
    if pool_type == LiquidityPoolType::ElasticSupply {
        ...
        if elastic_pools_amt > ELASTIC_MAX_POOLS {
            panic_with_error!(&e, PoolRouterError::ElasticPoolsOverMax);
        }
    }
