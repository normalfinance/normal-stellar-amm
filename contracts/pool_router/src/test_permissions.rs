...
use crate::testutils::{
    install_elastic_liq_pool_hash, install_liq_pool_hash, install_token_wasm, Setup,
};
...
fn test_set_hashes() {
    ...
    let elastic_pool_hash = install_elastic_liq_pool_hash(&setup.env);
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(
            router
                .try_set_elastic_pool_hash(&addr, &elastic_pool_hash)
                .is_ok(),
            is_ok
        );
    }
