...
// The `SinkTrait` trait provides the interface for interacting with a liquidity pool.
#[contractimpl]
impl SinkTrait for Sink {
    ...
    fn deposit(e: Env, user: Address, token: Address, amount: u128) {
        ...
        if get_is_killed_deposit(&e) {
            panic_with_error!(e, SinkError::FundDepositKilled);
        }

        // Whitelisted token check
        if !get_token_whitelist_status(&e, &token) {
            panic_with_error!(e, SinkError::UnsupportedToken);
        }
    ...
    fn withdraw(e: Env, user: Address, token: Address, amount: u128) {
        user.require_auth();
    ...
    fn pay_premium(e: Env, sender: Address, amount: u128) {
        ...
        if !get_premium_payer_status(&e, &sender) {
            panic_with_error!(&e, SinkError::NotAuthorized);
        }
    ...
    fn get_reserve(e: Env, token: Address) -> SinkReserve {
        get_reserve(&e, &token)
    }
...
//
#[contractimpl]
impl StakingTrait for Sink {
    // Deposits tokens into the Insurance Fund in exchange for share tokens representing stake ownership.
    //
    fn stake(e: Env, user: Address, token: Address, amount: u128) {
        user.require_auth();
        ...
        validate!(
            ...
            SinkError::IFWithdrawRequestInProgress
        );
    ...
    fn request_withdraw(e: Env, user: Address, token: Address, amount: u128) {
        ...
        if get_is_killed_request_withdraw(&e) {
            panic_with_error!(e, SinkError::FundRequestWithdrawKilled);
        }
        ...
        validate!(&e, n_shares > 0, SinkError::IFWithdrawRequestTooSmall);
    ...
    fn cancel_request_withdraw(e: Env, user: Address, token: Address) {
        ...
        validate!(
            &e,
            stake.last_withdraw_request_shares != 0,
            SinkError::NoIFWithdrawRequestInProgress
        );
    ...
    fn withdraw(e: Env, user: Address, token: Address) {
        ...
        if get_is_killed_withdraw(&e) {
            panic_with_error!(e, SinkError::FundWithdrawKilled);
        }
        ...
        validate!(
            ...
            SinkError::TryingToRemoveLiquidityTooFast
        );
        ...
        validate!(&e, new_reserve.balance > 0, SinkError::InvalidIFDetected);
...
impl AdminInterface for Sink {
    ...
    fn sync_optimal_insurance(e: Env, admin: Address) {
        ...
        let updated_optimal_insurance = if total_liquidity_imbalance <= 0 {
            ...
            u128::try_from(total_liquidity_imbalance).unwrap_or_else(|_| {
                panic_with_error!(&e, SinkError::ConversionOverflow);
            })
        };
    ...
    //
    // # Panics / Errors
    // * `SinkError::InsufficientCollateral` if the claim exceeds vault balance.
    // * `SinkError::InvalidIFDetected` if the payout fully depletes the Insurance Fund.
    fn file_claim(e: Env, admin: Address, token: Address, asset: Symbol) {
        ...
        match pool_details_result {
            Ok(Err(_pool_error)) => {
                panic_with_error!(&e, SinkError::QueryPoolFailed);
            }
            Err(_contract_error) => {
                panic_with_error!(&e, SinkError::QueryPoolFailed);
            }
            Ok(Ok(pool_info)) => {
                ...
                match pay_from_insurance_result {
                    Ok(Err(_pool_error)) => {
                        panic_with_error!(&e, SinkError::PayInsuranceClaimFailed);
                    }
                    Err(_contract_error) => {
                        panic_with_error!(&e, SinkError::PayInsuranceClaimFailed);
                    }
                    Ok(Ok(pay_from_insurance)) => {
                        if pay_from_insurance > 0 {
                            ...
                            validate!(
                                ...
                                SinkError::InsufficientCollateral
                            );
    ...
    fn add_token_whitelist(e: Env, admin: Address, token: WhitelistToken) {
        ...
        if get_token_whitelist_status(&e, &token.address) {
            panic_with_error!(&e, SinkError::AdminNotSet);
        }
...
#[contractimpl]
impl UpgradeableContract for Sink {
...
#[contractimpl]
impl TransferableContract for Sink {
