...
fn test_admin_transfer_ownership_too_early() {
    ...
    assert!(
        fee_collector.try_revert_transfer_ownership(&admin_new, &symbol_short!("Admin")).is_err()
    );
...
fn test_emergency_admin_transfer_ownership_too_early() {
    ...
    assert!(fee_collector.try_set_emergency_mode(&emergency_admin_new, &false).is_err());
    assert!(fee_collector.try_set_emergency_mode(&setup.emergency_admin, &false).is_ok());

    jump(&setup.env, ADMIN_ACTIONS_DELAY - 1);
    fee_collector.apply_transfer_ownership(
        &setup.admin,
        &Symbol::new(&setup.env, "EmergencyAdmin")
    );
}
...
fn test_emergency_admin_transfer_ownership_reverted() {
    ...
    jump(&setup.env, ADMIN_ACTIONS_DELAY + 1);
    fee_collector.revert_transfer_ownership(
        &setup.admin,
        &Symbol::new(&setup.env, "EmergencyAdmin")
    );
    fee_collector.apply_transfer_ownership(
...
fn test_emergency_admin_transfer_ownership() {
    ...
    assert!(fee_collector.try_set_emergency_mode(&emergency_admin_new, &false).is_ok());
    assert!(fee_collector.try_set_emergency_mode(&setup.emergency_admin, &false).is_err());
}
...
fn test_transfer_ownership_separate_deadlines() {
    ...
    assert_eq!(fee_collector.get_future_address(&symbol_short!("Admin")), setup.admin);
    ...
    assert_eq!(fee_collector.get_future_address(&symbol_short!("Admin")), admin_new);
    ...
    assert!(
        fee_collector.try_apply_transfer_ownership(&setup.admin, &symbol_short!("Admin")).is_err()
    );
...
fn test_commit_upgrade() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_commit_upgrade(&addr, &new_wasm).is_ok(), is_ok);
    }
}
...
fn test_apply_upgrade_emergency_admin() {
    ...
    fee_collector.commit_upgrade(&setup.admin, &install_dummy_wasm(&setup.env));
    jump(&setup.env, ADMIN_ACTIONS_DELAY + 1);
    assert!(fee_collector.try_apply_upgrade(&setup.emergency_admin).is_err());
}
...
fn test_set_emergency_mode_emergency_admin() {
    let setup = Setup::default();
    let fee_collector = setup.fee_collector;
    assert!(fee_collector.try_set_emergency_mode(&setup.admin, &false).is_err());
}
...
fn test_set_insurance_fund() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_set_insurance_fund(&addr, &insurance_fund).is_ok(), is_ok);
    }
}
...
fn test_set_fee_destination() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_set_fee_destination(&addr, &fee_destination).is_ok(), is_ok);
    }
}
...
fn test_set_buffer_fraction() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_set_buffer_fraction(&addr, &200).is_ok(), is_ok);
    }
}
...
fn test_set_lp_revenue_fraction() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_set_lp_revenue_fraction(&addr, &4000).is_ok(), is_ok);
    }
}
...
fn test_claim_fees() {
    ...
    for (addr, is_ok) in [
        ...
        assert_eq!(fee_collector.try_set_router(&addr, &new_router).is_ok(), is_ok);
    }
}
