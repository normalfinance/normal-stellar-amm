...
mod sophisticated_twap_attacks {
    ...
    fn test_twap_cascading_manipulation() {
        ...
        let mut twap_history = Vec::new(&e);
        twap_history.push_back(twap);
        
        for &step_price in manipulation_steps.iter() {
        ...
        for i in 1..twap_history.len() {
            assert!(twap_history.get(i).unwrap() > twap_history.get(i-1).unwrap());
        }
        
        // Final TWAP should be significantly manipulated
        let final_manipulation = twap_history.last().unwrap();
        let initial_twap = twap_history.get(0).unwrap();
...
mod twap_economic_exploitation {
    ...
    fn test_twap_insurance_premium_manipulation() {
        ...
        let mut twap_changes = Vec::new(&e);
        
        // Normal period: Stable prices
        for _ in 0..10 {
            timestamp += 30;
            let stable_price = 1000_0000000 + (timestamp % 10) * 100000; // Small variations
            let stable_price_u128 = stable_price as u128;
            let old_twap = twap;
            twap = calculate_new_twap(&e, stable_price_u128, timestamp, twap, timestamp - 30, period);
            twap_changes.push_back((twap as i128 - old_twap as i128).abs());
        }
        
        let normal_volatility: i128 = twap_changes.iter().sum::<i128>() / twap_changes.len() as i128;
        
        // Attack period: High volatility
        twap_changes = Vec::new(&e);
        for i in 0..10 {
            timestamp += 30;
...
mod advanced_twap_defense_evasion {
    ...
    fn test_twap_outlier_detection_evasion() {
        ...
        let mut price_history = Vec::new(&e);
        for _ in 0..20 {
            timestamp += 15;
            let normal_price = 1000_0000000 + ((timestamp % 100) * 100000); // ±1% variation
            let normal_price_u128 = normal_price as u128;
            price_history.push_back(normal_price_u128);
            twap = calculate_new_twap(&e, normal_price_u128, timestamp, twap, timestamp - 15, period);
        }
        
        // Calculate statistical bounds
        let mean = price_history.iter().sum::<u128>() / price_history.len() as u128;
        let mean_u128 = mean as u128;
