...
use crate::{
    ...
    storage_types::{
        HistoricalOracleData, NormalAction, OracleGuardRails, OracleStatus, OracleValidity,
        PriceDivergenceGuardRails, ValidityGuardRails,
    },
};
...
pub fn get_oracle_price(e: &Env, oracle: &Address, asset: &Address, now: u64) -> OraclePriceData {
    ...
    if oracle_price_data.timestamp + 24 * 60 * 60 < e.ledger().timestamp()
...
pub fn block_operation(
    ...
    let is_oracle_valid =
        is_oracle_valid_for_action(oracle_validity, Some(NormalAction::Rebalance));
...
pub fn is_oracle_valid_for_action(
    ...
    let is_ok = match action {
        Some(action) => match action {
            ...
            NormalAction::BufferPayout => !matches!(
                oracle_validity,
                OracleValidity::NonPositive | OracleValidity::TooVolatile
            ),
            NormalAction::InsuranceFundClaim => !matches!(
                oracle_validity,
                OracleValidity::NonPositive | OracleValidity::TooVolatile
            ),
        },
