...
fn test_admin_transfer_ownership_too_early() {
    ...
    assert!(calculator.try_revert_transfer_ownership(&admin_new, &symbol_short!("Admin")).is_err());
...
fn test_emergency_admin_transfer_ownership_too_early() {
    ...
    assert!(calculator.try_set_emergency_mode(&emergency_admin_new, &false).is_err());
    assert!(calculator.try_set_emergency_mode(&setup.emergency_admin, &false).is_ok());
...
fn test_emergency_admin_transfer_ownership() {
    ...
    assert!(calculator.try_set_emergency_mode(&emergency_admin_new, &false).is_ok());
    assert!(calculator.try_set_emergency_mode(&setup.emergency_admin, &false).is_err());
}
...
fn test_transfer_ownership_separate_deadlines() {
    ...
    assert_eq!(calculator.get_future_address(&symbol_short!("Admin")), setup.admin);
    ...
    assert_eq!(calculator.get_future_address(&symbol_short!("Admin")), admin_new);

    jump(&setup.env, ADMIN_ACTIONS_DELAY + 1 - 10);
    calculator.apply_transfer_ownership(&setup.admin, &Symbol::new(&setup.env, "EmergencyAdmin"));
    assert!(
        calculator.try_apply_transfer_ownership(&setup.admin, &symbol_short!("Admin")).is_err()
    );
...
fn test_commit_upgrade_third_party_user() {
    ...
    assert!(calculator.try_commit_upgrade(&user, &install_dummy_wasm(&setup.env)).is_err());
}
...
fn test_commit_upgrade_admin() {
    let setup = Setup::default();
    let calculator = setup.calculator;
    assert!(calculator.try_commit_upgrade(&setup.admin, &install_dummy_wasm(&setup.env)).is_ok());
}
...
fn test_apply_upgrade_emergency_admin() {
    ...
    calculator.commit_upgrade(&setup.admin, &install_dummy_wasm(&setup.env));
    jump(&setup.env, ADMIN_ACTIONS_DELAY + 1);
    assert!(calculator.try_apply_upgrade(&setup.emergency_admin).is_err());
}
...
fn test_set_emergency_mode_emergency_admin() {
    let setup = Setup::default();
    let calculator = setup.calculator;
    assert!(calculator.try_set_emergency_mode(&setup.admin, &false).is_err());
}
