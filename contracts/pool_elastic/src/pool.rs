...
pub fn is_swap_risk_reducing(e: &Env, oracle_price: u128, pool_price: u128, in_idx: u32) -> bool {
    if in_idx > 1 {
    ...
    let price_spread_i128 = oracle_price_i128.safe_sub(e, pool_price_i128);

    // Pool price lower than oracle, buys will be risk reducing
    if price_spread_i128 > 0 {
        if in_idx == 0 {
            false
        } else {
            true
        ...
        // Pool price higher than oracle, sells will be risk reducing
        if in_idx == 0 {
            true
        } else {
            false
