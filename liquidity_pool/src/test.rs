...
fn test() {
    ...
    liq_pool.deposit(&admin, &amount_to_deposit, &0);
    assert_eq!(e.auths()[0], (
        admin.clone(),
    ...
    // LP token
    let expected_share_amount = expected_mint_amount
        .fixed_mul_floor(amount_to_deposit as i128, PRICE_PRECISION_I128)
        .unwrap()
        .isqrt();
    ...
    let expected_swap_result = 4935_u128;
    ...
    assert_eq!(
        liq_pool.swap(&admin, &1, &0, &swap_in_amount, &expected_swap_result),
        expected_swap_result
    );

    let expected_new_mint = 9935_u128;
    ...
    //             liq_pool.address.clone(),
    //             Symbol::new(&e, "swap"),
    //             (&admin, 1_u32, 0_u32, swap_in_amount, expected_swap_result).into_val(&e),
    //         )),
    //         sub_invocations: std::vec![
    //             AuthorizedInvocation {
    //                 function: AuthorizedFunction::Contract((
    ...
    assert_eq!(token1.balance(&admin), expected_swap_result as i128);

    // Pool should have the initial mint amount, less the swap output,
    // plus the newly minted tokens to balance price
    assert_eq!(
    ...
    // User should have initial quote token, less deposit, less what
    // they swapped in
    assert_eq!(
    ...
    // ];
    let withdraw_amount = amount_to_deposit + swap_in_amount;
    // liq_pool.withdraw(&admin, &(expected_share_amount as u128), &withdraw_amount);
    // assert_eq!(e.auths()[0], (
    //     admin.clone(),
    //     AuthorizedInvocation {
    //         function: AuthorizedFunction::Contract((
    ...
    // assert_eq!(token1.balance(&admin), i128::MAX);
...
fn test_strict_receive() {
    ...
    setup.liq_pool.deposit(&user1, &desired_amount, &0);
    let swap_in_amount = 1_0000000_u128;
    let expected_swap_result = 4935_u128;
...
//     );
//     assert!(setup.liq_pool.try_estimate_swap_strict_receive(&1, &0, &99_7000000).is_err());
//     assert!(
//         setup.liq_pool.try_swap_strict_receive(&user1, &1, &0, &99_7000000, &100_0000000).is_err()
//     );
...
fn test_events() {
    ...
    liq_pool.deposit(&user1, &amount_to_deposit, &0);
    assert_eq!(
        ...
        vec![&e, (
            ...
            (
                amount_to_deposit as i128, // FIXME: share amount
                amount_to_deposit as i128,
            ).into_val(&e),
        )]
    );

    assert_eq!(liq_pool.swap(&user1, &1, &0, &100, &97), 98);
    assert_eq!(
        vec![&e, e.events().all().last().unwrap()],
        vec![&e, (
            liq_pool.address.clone(),
    ...
    // let amounts_out = liq_pool.withdraw(&user1, &amount_to_deposit, &0);
...
fn test_deposit_min_mint() {
    ...
    liq_pool.deposit(&user1, &1_000_000_000_000_0000000, &0);
    liq_pool.deposit(&user1, &1, &10);
}
...
fn test_zero_initial_deposit() {
    ...
    let user1 = setup.users[0].clone();
    setup.liq_pool.deposit(&user1, &0, &0);
}
...
fn test_zero_deposit_ok() {
    ...
    let liq_pool = setup.liq_pool;
    let user1 = setup.users[0].clone();
    liq_pool.deposit(&user1, &100, &0);
    liq_pool.deposit(&user1, &0, &0);
}
...
fn test_custom_fee() {
    ...
    for fee_config in [
        ...
        liqpool.deposit(&setup.users[0], &100_0000000, &0);
...
//     liq_pool.deposit(&users[0], &100, &0);
...
//     jump(&env, 30);
//     assert_eq!(liq_pool.claim(&users[0]), total_reward_1 / 2);
//     liq_pool.deposit(&users[1], &100, &0);
//     jump(&env, 100);
...
//     let locked_token_admin_client = get_token_admin_client(
//         &env,
//         &setup.reward_boost_token.address.clone()
//     );
...
//     setup.reward_boost_feed.set_total_supply(&setup.operations_admin, &20_000_0000000);
//     liq_pool.deposit(&users[1], &100, &0);
...
//     setup.reward_boost_feed.set_total_supply(&setup.operations_admin, &new_locked_supply);
...
//         new_locked_supply
//     );
//     assert_eq!(new_rewards_info.get(symbol_short!("supply")).unwrap() as u128, supply);
...
//     liq_pool.deposit(&users[0], &100, &0);
//     jump(&env, 59);
//     liq_pool.deposit(&users[1], &1000, &0);
//     jump(&env, 100);
...
//     liq_pool.deposit(&users[0], &900, &0);
//     liq_pool.deposit(&users[1], &100, &0);
...
//     jump(&env, 20);
//     liq_pool.deposit(&users[2], &1000, &0);

//     jump(&env, 10);
//     liq_pool.withdraw(&users[2], &1000, &1000);
...
//     liq_pool.deposit(&first_user, &1_000_000_000_000_0000000, &0);
//     jump(&env, 1);

//     for i in 1..iterations_to_simulate as usize {
//         let user = &users[i % 10];
//         liq_pool.deposit(user, &1_000_000_000_000_0000000, &0);
//         jump(&env, 1);
//     }
...
fn test_zero_swap() {
    ...
    liq_pool.deposit(&user1, &desired_amount, &0);
...
fn test_swap_killed() {
    ...
    assert_eq!(
        liq_pool.try_swap(&user1, &1, &0, &100, &0).unwrap_err(),
...
fn test_deposit_killed() {
    ...
    assert_eq!(
        liq_pool.try_deposit(&user1, &desired_amount, &0).unwrap_err(),
...
//     liq_pool.deposit(&user2, &1_000_0000000, &0);
//     assert_eq!(liq_pool.get_reserves(), Vec::from_array(&e, [1_100_0000000, 1_100_0000000])); // FIXME: update token1 reserve to be expected output

//     assert_eq!(liq_pool.get_reserves(), Vec::from_array(&e, [1_100_0000000, 1_100_0000000]));
...
//     liq_pool.deposit(&user1, &1_000_000_0000000, &0);
